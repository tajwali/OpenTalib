...
## Master Vibe-Coding Instructions

### Architectural Standards (The "Good Citizen" Rule)
- **Design Patterns:** Do not write "tactical tornado" code. Use established patterns (e.g., Factory, Singleton, Hooks) to ensure the code is logical for humans.
- **Dependency Management:** Be aggressive about "Zero-Bloat." If I ask for a component, do not import a 20MB library for one button. Prefer native CSS, Tailwind, or lightweight alternatives.
- **UI Consistency:** Always check the /components or /ui folder first. Reuse existing logic before generating new UI code to prevent "style spaghetti."

### Quality & Security (The "Expert-in-the-Loop" Check)
- **Test-First Logic:** For every feature or refactor, you must provide corresponding unit/integration tests (e.g., Vitest, Jest, Playwright).
- **Security First:** Never hardcode secrets. Always use environment variable patterns (process.env). If a package you suggest has known vulnerabilities, find an alternative.
- **Performance:** Refactor for token efficiency. Minimize data payloads and optimize loops to keep the app lean and cost-effective.

### Open-Source Compliance (The "Linux Law")
- **Commit Metadata:** When providing commit messages or summaries, include the tag: Assisted-by: [Your Agent Name] ([Model Version]).
- **Provenance:** Ensure generated code adheres to standard open-source licenses (MIT/GPL). Avoid "leaking" code snippets that carry restrictive proprietary licenses.
- **The "Signed-off-by" Boundary:** You are an assistant, not a legal entity. Never generate or sign the Signed-off-by: tag; leave that for me, the human controller.

### Documentation & Clarity
- **The "Why" vs. the "What":** Don't just comment on what the code does; use comments to explain the intent behind complex logic.
- **README updates:** If a change requires new setup steps or environment variables, automatically provide the updated README.md or .env.example snippet.