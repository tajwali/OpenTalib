...
- Do not assume the OpenClaw agent's own model or API key will be reused by OpenTalib.
- OpenTalib classroom generation uses OpenTalib server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only OpenTalib server-side config files may control provider selection and defaults.
...
Use this when the user has not installed OpenTalib yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify OpenTalib

After the user has chosen a startup mode and configured keys, start OpenTalib using the chosen method, then verify the service with `GET {url}/api/health`.
